...
@Component
public class FootballNormalizer implements SportNormalizer {

    private static final Logger log = LoggerFactory.getLogger(FootballNormalizer.class);
    private final ObjectMapper objectMapper;

    public FootballNormalizer(ObjectMapper objectMapper) {
    ...
    public Optional<SportEvent> normalize(String json) throws Exception {
        ...
        if (!responseArray.isArray() || responseArray.isEmpty()) {
            log.info("No live football data, skipping");
            return Optional.empty();
        }

        JsonNode fixture = responseArray.path(0);
        JsonNode fixtureIdNode = fixture.path("game").path("id");
